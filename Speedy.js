G.AddData({
	...
	func: function () {
			...
			G.createTopInterface = function () {
				var str = '' +
					...
					G.button({
						...
						onclick: function () {
							if (G.fastTicks > 0) { G.setSetting('paused', 0); G.setSetting('fast', 1); }

						}
					}) +
					G.button({
						id: 'customSpeedButton',
						...
						onclick: function () {
							if (G.fastTicks > 0) {
								G.setSetting('paused', 0); G.setSetting('fast', 0);
								var sp = prompt("Please enter the custom speed you want to run this game.");
		...
		new G.Res({
			name: 'fasttickdebug', hidden: true,
